...
## configs 
module purge
module load samtools/1.10 intel/21.2.0 mkl/21.3.0 R/4.3.0 gcc/11.2.1
...
## generate genotype mapping
Rscript src/match_hybrids.R

## combine chromosomes
bcftools concat data/chr_{1..10}_imputed.vcf.gz --threads 8 -Oz -o output/merged.vcf.gz

## keep only snps and filter MAF > 0.05
bcftools view -m2 -M2 -v snps -q 0.05:minor output/merged.vcf.gz -Oz -o output/merged_m005_snps.vcf.gz
